...
@api.route('/login')
def login():
    return google_oauth.authorize(callback=url_for('users.authorized', _external=True))
...
from runtime.config.flask_app_config import CONSUMER_KEY, CONSUMER_SECRET
