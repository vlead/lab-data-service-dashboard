...
* Introduction 
  This document illustrates building of the REST API. With the use of Oauth Service
...
*** =Login API=
    ...
    thereby it will forward request to =oauth=. User must sign in using it's 
    google credentials which in the oauth service.
...
*** Oauth Callback response =API=
    - Once the Oauth verfies the user credentials to will send callback.In the callback the service will fetch users data from the check_login endpoint of the oauth service. 
