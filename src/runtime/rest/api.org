...
def index():
	...
	if session.get('type') == "guest":
		return render_template('guest_home.html', data = data)
...
def loggedin():
    if 'google_token' in session:
        return jsonify(success=True, session=session['type']), 200
    else:
...
def delete_user():
    try:
        ...
        try:
            ...
            return jsonify(success=False, message="%s not sent in the request" % e.args), 100

    user = User.query.filter(User.email == email)
    user_todel = user.first()
    
    if user_todel is None:
        return jsonify(success=False, message="This email does not exists"), 300
    elif (user_todel == current_user):
    	return jsonify(success=False, message="This is not an valid operation"), 600
    elif (user_todel.role == 'admin' and len(user) == 1):
    	return jsonify(success=False, message="At least one admin must be present"),700
...
<body>
    <header id="top" class="header">
        <div class="text-vertical-center">
            ...
            <br>
            <button class= "btn btn-dark btn-lg" onclick="window.location.href='{{auth_url}}'">LOGIN</button>
        </div>
    </header>
</button>
...
<body id="body">
  ...
  <div class="navbar-header">
        <a class="navbar-brand" href="#">LAB DATA SERVICE</a>
      </div>
        ...
        <div class="navbar-custom-menu">
          <ul class="nav navbar-nav navbar-right">
            ...
            <li class="dropdown">
              <button class="navbar-btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown" >{{data['email']}}
            ...
            <li> <span class="col-sm-4"></span> </li>
              <!--
            </div>
...
** Admin Home
#+NAME: admin_home
#+BEGIN_SRC html :tangle app/templates/admin_home.html :eval no :noweb yes
<!DOCTYPE html>
<html>
<head>
    <title>HOME</title>
...
<div class="col-lg-3 col-md-3 col-sm-4" style="float:right">
    <div class="list-group table-of-contents">  
      <a class="list-group-item" href='/'> Add User</a>
      <a class="list-group-item" href='/'> Add Role</a>
      <a class="list-group-item" href='/'> View Users</a>
      <a class="list-group-item" href='/'> View Roles</a>
      <a class="list-group-item" href='/'> Edit User</a>
      <a class="list-group-item" href='/'> Delete User</a>
      <a class="list-group-item" href="/">Delete Role</a>
    </div>
</div>

  <!--<div style="text-align:center">-->
  <div style="padding-top:10%; text-align:center; float:center">
  <h3> Hello, {{data['name']}}! </h3>
  <h4> Email id - {{data['email']}} </h4>
  <h4> Role - {{data['role']}} </h4>
  </div>
  ...
  <div id="container" class="clear">
    <section id="fof" class="clear">
      <div class="positioned">
        ...
        <p>Jokes Aside<br/>
          Make sure you know what you are trying to do. If you think this is an error then contact us by dropping an e-mail</p>
          </div>
    </section>  
  </div>
</div>
...
** Error 405 page
#+NAME: 404
#+BEGIN_SRC html :tangle app/templates/405.html
<!DOCTYPE html>
<html lang="en-US">
<head>
    <meta charset="UTF-8">
  ...
  <div id="container" class="clear">
    <section id="fof" class="clear">
      <div class="positioned">
        <div class="hgroup">
          <h1>Ohh NO!</h1>
          <h2>The method you used to access this route is not allowed. Apologies.</h2>
        </div>
        <p>Jokes Aside<br/>
          Make sure you know what you are trying to do. If you think this is an error then contact us by dropping an e-mail</p>
                </div>
...
from runtime.rest.app import app, db, requires_guest_auth, requires_admin_auth
from runtime.rest.app.users.db import User, Roles
import os
import json
import re
