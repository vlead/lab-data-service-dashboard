...
var populate_tables_user = function() {
  console.log($('#user-table').find('tr:not(:first)'));
  console.log($('#user-table').find('td'));
  $('#user-table').find('td').remove();
  ...
  for (var i in users_List) {
      ...
      var newCell = newRow.insertCell(-1);
      var updateButton = document.createElement('button');
      updateButton.setAttribute('id','remove-user-pre-sbt-'+i);
      updateButton.setAttribute('email', users_List[i].email);
      updateButton.appendChild(document.createTextNode('Delete'));
      newCell.appendChild(updateButton);
  }
...
var populate_tables_role = function() {
  ...
  for (var i in roles_List) {
      ...
      var newCell = newRow.insertCell(-1);
      var updateButton = document.createElement('button');
      updateButton.setAttribute('id','remove-role-pre-sbt-'+i);
      updateButton.setAttribute('role',roles_List[i]);
      updateButton.appendChild(document.createTextNode('Delete'));
      newCell.appendChild(updateButton);
  }
