...
Whenever =hash= is changed,the change is detected and the function
checks if the new hash is valid or not. If the new hash is valid,
we are navigated to the new hash, else an error is thrown. 
