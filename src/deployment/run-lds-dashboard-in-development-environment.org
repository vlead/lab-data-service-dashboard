...
* Initial make
  Run make from =lab-data-service-dashboard= directory to build the application.  The
...
* Run Make again
  The =make= again and time it should pass all the test cases. Change directory to =lab-data-service-dashboard=
  and run =make=.
...
* Set SQLALCHEMY_DATABASE_URI
  ...
  =build/code/runtime/config/flask_app_config.py= so that the application
...
* Insert Dafault data
  Go to =build/code/deployment= and open ~dp_setup.py~. Change the
